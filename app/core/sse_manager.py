...
ClientCallback = Callable[[Dict[str, Any]], None]


class SSEManager:
    """Manage in-memory SSE subscribers and recent incident history."""

    def __init__(self):
        self._clients: Dict[str, List[ClientCallback]] = defaultdict(list)
        self._incident_events: Dict[str, List[Dict[str, Any]]] = defaultdict(list)
        self._max_events_per_incident = 250
        self._lock = threading.Lock()

    def connect(self, client_id: str, callback: ClientCallback):
        """Register a callback for a specific client."""

        with self._lock:
            self._clients[client_id].append(callback)

    def disconnect(self, client_id: str, callback: ClientCallback | None = None):
        ...
        with self._lock:
            ...
            callbacks = self._clients.get(client_id)
            if not callbacks:
                return
            ...
            if not callbacks:
                ...
                self._clients[client_id] = callbacks

    def broadcast(self, event_type: str, data: Any, client_id: str | None = None):
