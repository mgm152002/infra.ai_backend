...
def test_process_incident_flow():
    ...
    # Patch dependencies
    with (
        patch("app.services.incident_service.supabase", mock_supabase),
        patch("app.services.incident_service.call_llm", side_effect=mock_call_llm),
        patch(
            "app.services.incident_service.fetch_ssh_key_content", return_value="fake-key-content"
        ),
        patch(
            "app.services.incident_service.run_shell_command",
